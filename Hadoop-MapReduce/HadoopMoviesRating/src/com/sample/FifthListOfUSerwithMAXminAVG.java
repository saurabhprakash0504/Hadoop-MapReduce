...
public class FifthListOfUSerwithMAXminAVG {
	...
	static class MapperMovieRating extends Mapper<LongWritable, Text, Text, DoubleWritable> {
		public void map(LongWritable key, Text value, Context con) throws IOException, InterruptedException {
			...
			IntWritable outKey = new IntWritable(Integer.parseInt(user));
			DoubleWritable outValue = new DoubleWritable(movieRating.length() > 0 ? Double.parseDouble(movieRating) : 0);
			/*con.write(new Text("max"+key.toString()), outValue);
			con.write(new Text("min"+key.toString()), outValue);
			con.write(new Text("avg"+key.toString()), outValue);
			*/
			con.write(new Text(key.toString()), outValue);
